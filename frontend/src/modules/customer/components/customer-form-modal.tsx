...
import { getCustomer, createCustomer, updateCustomer } from "@/modules/customer/customer.api";
import { CACHE_KEYS } from "@/lib/constants";
import { Tag, Mail, Phone, Building2, Globe, FileText, Loader2, Save, Plus } from "lucide-react";
...
export function CustomerFormModal({ open, onOpenChange, customerId }: Props) {
  const queryClient = useQueryClient();
  const isEdit = !!customerId;

  const { data: customer, isLoading: loadingCustomer } = useQuery({
    queryKey: CACHE_KEYS.CUSTOMER(customerId!),
  ...
  const {
    ...
    defaultValues: {
      ...
      taxId: "", website: "", notes: "", status: "active",
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (data: CustomerFormValues) => updateCustomer(customerId!, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: CACHE_KEYS.CUSTOMERS });
      queryClient.invalidateQueries({ queryKey: CACHE_KEYS.CUSTOMER(customerId!) });
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div className="space-y-1">
                      ...
                      <Input id="code" placeholder="CUST-001" className="h-8 text-sm" {...register("code")} />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div className="space-y-1">
                      ...
                      <Input id="email" type="email" placeholder="john@example.com" className="h-8 text-sm" {...register("email")} />
