...
import {
  listChecklists, deleteChecklist,
  type Checklist,
} from "@/modules/quality/quality.api";
import { ChecklistFormModal } from "@/modules/quality/components/checklist-form-modal";
...
export default function ChecklistsPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: [...CACHE_KEYS.QC_CHECKLISTS, { page, limit: 20, search }],
    queryFn: () => listChecklists({ page, limit: 20, search }),
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">QC Checklists</h2>
          <p className="text-muted-foreground text-sm">Define inspection checklists for quality control</p>
        </div>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <EmptyState icon={<ClipboardList className="h-12 w-12" />} title="Failed to load checklists" description="Try refreshing the page" />
            ...
            <EmptyState icon={<ClipboardList className="h-12 w-12" />} title="No checklists found" description="Add a checklist to get started" />
          )}
        </CardContent>
      </Card>
